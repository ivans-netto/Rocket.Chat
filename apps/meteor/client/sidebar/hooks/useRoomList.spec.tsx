...
// Mock RoomManager (no room open in these tests) to avoid loading its real module graph, which imports
// non-JS assets jest can't transform.
...
it('should return groupsCount with the correct count', async () => {
	const { result } = renderHook(() => useRoomList({ collapsedGroups: [] }), {
		...
		wrapper: getWrapperSettings({ sidebarShowCustomCategories: false, sidebarShowFavorites: false }).build(),
	});
