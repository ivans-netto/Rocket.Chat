...
import { useSystemGroupsOrder } from './useSystemGroupsOrder';
...
// Leading icon per system group, so system groups align with custom categories (which use a folder icon).
...
export type SidebarRoomListGroup = {
	...
	/** Leading icon: folder for custom categories, the type icon for system groups. */
	icon: IconName;
...
export const useRoomList = ({ collapsedGroups }: { collapsedGroups?: string[] }): useRoomListReturnType => {
	...
	// Custom categories toggle; default on.
