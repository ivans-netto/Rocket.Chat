...
|---|---|---|
| `sidebarAllGroupsOrder` | `useAllGroupsOrder` | Unified order of all groups (custom + system). When empty, custom categories appear first then system groups. |
| `sidebarSystemGroupsOrder` | `useSystemGroupsOrder` | Order of system groups independently (used inside the unified sort). |
...
| Reorder | `reorderCategory(id, 'up' \| 'down')` | Swaps adjacent entries in the preference array; clamped at the ends. |
...
`useRoomList.ts` builds a `rid → categoryId` map first. A room assigned to a custom category is added
**only** to that category's set and `return`s before the system-group bucketing — so it never appears twice.
When the `sidebarShowCustomCategories` preference is off, the map is ignored and rooms fall through to their
system groups. Custom categories **persist even when empty** (an empty category renders with 0.4 opacity and
reserves no rows for rooms).

### Group ordering

`useAllGroupsOrder` applies a unified sort over all groups (custom + system combined) after the debounce,
so Move up / Move down in the category kebab takes effect immediately. Keys not yet in the saved order get
rank `-1` (sorted first), so newly created categories always surface at the top.

### Collapsed display

When a group is collapsed, it shows only its unread rooms when "Show unreads" is on; otherwise the rooms are
hidden and the collapser badge shows the total unread count. When open and "Keep unreads on top" is set,
unread rooms are stable-partitioned to the top (preserving the configured sort within each partition). This
logic lives in `useRoomList.ts`'s `makeGroup` function.
...
For non-livechat rooms, `RoomMenu.tsx` strips the `toggleFavorite` action and adds a **"Category"** section
with a `MenuSubmenuTrigger` ("Move to ▸"). This uses fuselage's native `Menu` + `MenuSubmenuTrigger`
(react-aria submenu) — no custom popover needed.

The submenu has two sections:
1. Category targets — Favorites + each custom category, each with a check-mark `addon` when it is the
   current grouping. Built by `useRoomCategoryItems`.
2. Utility actions — "New category" (opens `CategoryFormModal` seeded with the room) and
   "Remove from {category}" (danger item, only shown when the room is currently grouped).
...
Reorder actions close the menu before moving. The toggle-switch rows call `toggleCustomShowUnreads` /
`toggleSystemShowUnreads` depending on whether a `category` prop is present.

### Room-header grouping menu — `RoomGroupingMenu.tsx`

A `GenericMenu` placed before the room title in `RoomHeader`. Its icon reflects the current grouping:
`star-filled` for a favorite, `folder` for a room in a custom category, `star` otherwise.
...
`useCreateNewMenu.ts` adds a "Category" item (icon: `folder`) below the create-room items in the navbar `+`
menu. It calls `useCategoryModals().openCreate()` with no room argument.

## Translations

Keys added to `packages/i18n/src/locales/en.i18n.json`:
...
| `Show_unreads` / `When_closed` / `When_opened` / `Keep_unreads_on_top` | Show unreads / When closed / When opened / Keep unreads on top |
...
| System-group order | `apps/meteor/client/sidebar/hooks/useSystemGroupsOrder.ts` |
